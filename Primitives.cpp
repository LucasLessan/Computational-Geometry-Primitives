...
// Wherever there's a comment with "This is to improve readability", this means that we create variables to reduce
// clutter.
//
...
int Primitives::turn_direction(Segment s1, Segment s2) {
  ...
  // Note that this is the same as s1 + s2, if treated as vectors, in other words, if you translate s1 to the origin,
  // and s2 accordingly, so that s1._p2 = s2._p1 holds.
...
bool Primitives::intersect(Segment s1, Segment s2) {
  ...
  // This is so we can test if the left (and right) endpoints of s1 are on opposite sides in relation to s2
  double d1 = cross_product(Segment(p3, p1), s2);
  double d2 = cross_product(Segment(p3, p2), s2);

  // This is so we can test if the left (and right) endpoints of s2 are on opposite sides in relation to s1
  double d3 = cross_product(Segment(p1, p3), s1);
  double d4 = cross_product(Segment(p1, p4), s1);

  // This tests if each endpoint of s1 and s2 are on opposite sides in relation to the other line segment
  if (inverse_signs(d1, d2) && inverse_signs(d3, d4))
    return true;

  // Checks if any endpoint of s1 lies on s2, given that p1 or p2 are colinear with s2 because d1 or d2 is zero
  if (d1 == 0 && intersect(p1, s2)) return true;
  if (d2 == 0 && intersect(p2, s2)) return true;

  // Checks if any endpoint of s2 lies on s1, given that p3 or p4 are colinear with s1 because d3 or d4 is zero
