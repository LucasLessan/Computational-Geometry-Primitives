...
class Primitives {
  public:
    ...
    // Returns 1 if s1 is clockwise from s2 in relation to the origin, -1 if s2 is clockwise from s1 and 0 if they're
    // colinear.
    static int clockwise(Segment s1, Segment s2); 

    // Given consecutive line segments s1 and s2, returns 1 if s2 turns left from s1, -1 if s2 turns right and 0 if s2
    // goes straight from s1.
  ...
  private:
    ...
    // Given a line segment s, it translates s to the origin (treating s._p1 as the origin) and returns the point that
    // represents the new vector.
